...
/**
 * Function which calculate maximum number of cakes Pete can bake
  ...
  var count;
    for (key in recipe) {
      if (!available.hasOwnProperty(key)) {
        ...
        number.push(parseInt(available[key] / recipe[key]));
      }
      
    }
    count = Math.min.apply(Math, number)
    console.log(count);
    return count;
}
//Examples:
...
cakes({bread: 10, mayonnaise: 2}, {bread: 119, mayonnaise: 2}); // must return 1
